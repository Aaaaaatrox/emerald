...
void LoadPlayerBag(void)
{
    ...
    for (i = 0; i < MAIL_COUNT; i++)
        ...
        for (i = 0; i < BAG_MEDICINE_COUNT; i++)
            gLoadedSaveData.medicine[i] = gSaveBlock3Ptr->bagPocket_Medicine[i];
    #endif
    
    #if POCKET_BATTLE_ITEMS != DEFAULT_BATTLE_ITEMS_POCKET
        // load player battle items.
        for (i = 0; i < BAG_BATTLEITEMS_COUNT; i++)
            gLoadedSaveData.battleItems[i] = gSaveBlock3Ptr->bagPocket_BattleItems[i];
    #endif
    
    #if POCKET_POWER_UP != DEFAULT_POWER_UP_POCKET
        // load player power up items.
        for (i = 0; i < BAG_POWERUP_COUNT; i++)
            gLoadedSaveData.powerUp[i] = gSaveBlock3Ptr->bagPocket_PowerUp[i];
    #endif
    
    #if POCKET_MAIL != DEFAULT_MAIL_POCKET
        // load player mail.
        for (i = 0; i < BAG_MAIL_COUNT; i++)
            gLoadedSaveData.bagMail[i] = gSaveBlock3Ptr->bagPocket_Mail[i];
    #endif
    
    #if POCKET_TREASURES != DEFAULT_TREASURES_POCKET
        // load player treasures.
        for (i = 0; i < BAG_TREASURES_COUNT; i++)
            gLoadedSaveData.treasures[i] = gSaveBlock3Ptr->bagPocket_Treasures[i];
    #endif
    
    #if POCKET_MEGA_STONES != DEFAULT_MEGA_STONES_POCKET
        // load player mega stones.
        for (i = 0; i < BAG_MEGASTONES_COUNT; i++)
            gLoadedSaveData.megastones[i] = gSaveBlock3Ptr->bagPocket_MegaStones[i];
    #endif
...
void SavePlayerBag(void)
{
    ...
    for (i = 0; i < MAIL_COUNT; i++)
        ...
        for (i = 0; i < BAG_MEDICINE_COUNT; i++)
            gSaveBlock3Ptr->bagPocket_Medicine[i] = gLoadedSaveData.medicine[i];
    #endif
    
    #if POCKET_BATTLE_ITEMS != DEFAULT_BATTLE_ITEMS_POCKET
        // save player battle items.
        for (i = 0; i < BAG_BATTLEITEMS_COUNT; i++)
            gSaveBlock3Ptr->bagPocket_BattleItems[i] = gLoadedSaveData.battleItems[i];
    #endif
    
    #if POCKET_POWER_UP != DEFAULT_POWER_UP_POCKET
        // save player power up items.
        for (i = 0; i < BAG_POWERUP_COUNT; i++)
            gSaveBlock3Ptr->bagPocket_PowerUp[i] = gLoadedSaveData.powerUp[i];
    #endif
    
    #if POCKET_MAIL != DEFAULT_MAIL_POCKET
        // save player mail.
        for (i = 0; i < BAG_MAIL_COUNT; i++)
            gSaveBlock3Ptr->bagPocket_Mail[i] = gLoadedSaveData.bagMail[i];
    #endif
    
    #if POCKET_TREASURES != DEFAULT_TREASURES_POCKET
        // save player treasures.
        for (i = 0; i < BAG_TREASURES_COUNT; i++)
            gSaveBlock3Ptr->bagPocket_Treasures[i] = gLoadedSaveData.treasures[i];
    #endif
    
    #if POCKET_MEGA_STONES != DEFAULT_MEGA_STONES_POCKET
        // save player mega stones.
        for (i = 0; i < BAG_MEGASTONES_COUNT; i++)
            gSaveBlock3Ptr->bagPocket_MegaStones[i] = gLoadedSaveData.megastones[i];
    #endif
    
    #if POCKET_Z_CRYSTALS != DEFAULT_Z_CRYSTALS_POCKET
        // save player z-crystals.
        for (i = 0; i < BAG_ZCRYSTALS_COUNT; i++)
            gSaveBlock3Ptr->bagPocket_ZCrystals[i] = gLoadedSaveData.zcrystals[i];
    #endif
